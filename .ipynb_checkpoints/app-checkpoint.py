...
if st.sidebar.button("📊 TEST 1 : Patient Normal", use_container_width=True):
    st.session_state.age = 0.038
    st.session_state.sex = 0.051
...
st.write("Saisissez les valeurs des 10 variables normalisées et cliquez sur **Prédire**.")

# Création des inputs avec session_state
col1, col2 = st.columns(2)

with col1:
    age = st.number_input("Age", value=st.session_state.get('age', 0.0), step=0.01, format="%.3f")
    sex = st.number_input("Sex", value=st.session_state.get('sex', 0.0), step=0.01, format="%.3f")
    bmi = st.number_input("BMI", value=st.session_state.get('bmi', 0.0), step=0.01, format="%.3f")
...
with col2:
    s2 = st.number_input("s2", value=st.session_state.get('s2', 0.0), step=0.01, format="%.3f")
    s3 = st.number_input("s3", value=st.session_state.get('s3', 0.0), step=0.01, format="%.3f")
    s4 = st.number_input("s4", value=st.session_state.get('s4', 0.0), step=0.01, format="%.3f")
    s5 = st.number_input("s5", value=st.session_state.get('s5', 0.0), step=0.01, format="%.3f")
    s6 = st.number_input("s6", value=st.session_state.get('s6', 0.0), step=0.01, format="%.3f")
